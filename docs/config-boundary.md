...
| `MAX_LEARNSETS`                         | Registry capacity            | Learnset storage                        | None                                     |
...
| `MAX_CRITICAL_HIT_VALUE`                | Runtime semantic restriction | Critical-hit random-roll upper bound    | Not currently policy-configurable        |
| `MAX_ACCURACY_HIT_VALUE`                | Runtime semantic restriction | Accuracy random-roll upper bound        | Not currently policy-configurable        |
| `RANDOMIZATION_MULTIPLIER_MAX_VALUE`    | Runtime semantic restriction | Random damage multiplier upper bound    | Not currently policy-configurable        |
| `MAX_STAT_STAGES`                       | Runtime semantic restriction | Maximum positive or negative stat stage | Not currently policy-configurable        |
| `MAX_IV_STAT_VALUE`                     | Runtime semantic restriction | Maximum individual value                | Not currently policy-configurable        |

The remaining `MIN_*` and value constants in `constants.h` are also runtime mechanics, but they are outside this `MAX_*` capacity audit. They remain compile-time constants because the current ruleset model does not expose them as configurable policy fields.
...
4. New configurable runtime rules belong in `RulesetPolicy`, with validation, defaults, and tests added at that boundary.
