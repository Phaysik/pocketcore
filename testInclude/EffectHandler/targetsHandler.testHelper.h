/*! @file targetsHandler.testHelper.h
	@brief Test helper for dealing with TargetsHandler concepts.
	@date 09/01/2026
	@since 0.12.14
	@version 0.12.14
...
#ifndef TEST_INCLUDE_EFFECT_HANDLER_TARGETS_HANDLER_TEST_HELPER_H
#define TEST_INCLUDE_EFFECT_HANDLER_TARGETS_HANDLER_TEST_HELPER_H
...
namespace PocketCore::Testing
{
	...
	constexpr bool hasTargetsMultiplier(const EffectContext &context)
	{
		return std::ranges::any_of(context.getActiveMultipliers(), [](const std::pair<MultiplierID, double> &multiplierPair) {
			return multiplierPair.first == toMultiplierID(BuiltinMultiplierID::Targets)
				&& approximatelyEqualAbsRel(multiplierPair.second, TARGETS_HIT_MULTIPLIER_VALUE);
		});
	}
