...
#ifndef INCLUDE_EFFECT_SET_MISTY_TERRAIN_HANDLER_H
#define INCLUDE_EFFECT_SET_MISTY_TERRAIN_HANDLER_H
...
namespace PocketCore::Effect
{
	...
	/*! @class SetMistyTerrainHandler EffectHandler/setMistyTerrainHandler.h
		@brief Sets the battle-wide terrain to misty terrain.
		@details The handler resolves misty terrain against the active terrains through the provider's terrain registry and applies any
	   registered terrain interactions. Misty terrain is inserted into a free slot when one is available; once @ref
	...
	*/
	...
	{
		public:
			/*! @brief Applies misty terrain to the active terrains according to its registered interactions.
				...
				@param[in] provider The registry provider whose terrain registry resolves misty terrain's interaction metadata.
	...
	/*! @brief Applies misty terrain to the active terrains according to its registered interactions.
		...
		@param[in,out] context The effect context, which is not inspected or modified.
		@param[in] provider The registry provider whose terrain registry resolves misty terrain's interaction metadata.
		@post @p state reports misty terrain among its active terrains unless a blocking interaction prevented insertion or the terrain
	   list was full and @ref RulesetPolicy::mReplaceTerrainWhenFull is false.
		@since 0.12.43
		@version 0.12.43
	*/
	inline void applySetMisty(BattleState &state, EffectContext &context, const RegistryProvider &provider)
	{
		SetMistyTerrainHandler{}.apply(state, context, provider);
	}
} // namespace PocketCore::Effect
