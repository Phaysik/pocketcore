...
- Changed Pokemon-instantiation diagnostics to identify `PokemonRegistry::instantiate()` after the API's relocation from the configuration facade, and retained error messages as `std::string_view` values instead of allocating temporary strings.
- Added the missing direct `<sstream>` include to the Pokemon helper test that constructs an `std::ostringstream`.

### Fixed

- Fixed `PokemonRegistry::instantiate()` to reject a null `PokemonInstantiationDependencies` pointer before dereferencing it; missing dependency registries continue to return a structured error.
- Fixed nature, ability, item, and move validation control flow to propagate `std::expected` errors only when validation fails. The 0.12.26 implementation inverted this condition and attempted to read `error()` from successful validations.

## [0.12.28] - 2026-09-12

### Added

- Added `Pokemon/pokemonHelpers.h` and its source implementation as the home of `printPokemonWithNames()`, separating registry-resolved formatting from the core `Pokemon` type and numeric stream operator.
- Added an out-of-line `PokemonRegistry` implementation and a dedicated Pokemon-helper Catch2 suite that verifies complete registry-resolved output, including stats, IVs, EVs, types, natures, abilities, items, statuses, moves, and PP.

### Changed

- Moved `PokemonInstantiationDependencies`, `instantiate()`, and the nature, ability, item, and move metadata validators from `PokemonRegistryConfiguration` to `PokemonRegistry`. Runtime callers can now instantiate a Pokemon directly from a read-only registry without going through the mutation facade.
- Changed the configuration facade back to registry mutation and lookup responsibilities only; instantiation still combines species metadata with dependency registries and caller-supplied or generated IV, EV, nature, ability, item, and move selections.
- Moved registry-aware Pokemon formatting declarations out of `pokemon.h`, implementation out of `pokemon.cpp`, and coverage out of `pokemon.test.cpp` into the new helper module and suite.

## [0.12.27] - 2026-09-12

### Changed

- Changed `getRandomAbilityIDFromConfiguration()`, `getRandomItemIDFromConfiguration()`, `getRandomMoveIDFromConfiguration()`, and `getRandomNatureIDFromConfiguration()` to accept their read-only runtime registry types instead of configuration facades.
- Replaced configuration-header dependencies in the four random helper headers with the corresponding registry headers. Selection behavior remains an inclusive random stable ID from one through the registry's registered count.

## [0.12.26] - 2026-09-11

### Added

- Added ability, item, move, and nature helper headers with two random-selection APIs per domain: one selects a non-None built-in ID from the closed built-in range, and one selects from the IDs present in a supplied configuration.
- Added reusable Pokemon-instantiation validators for nature, ability, item, and move arrays. Each resolves metadata through `PokemonInstantiationDependencies` and reports the corresponding `NatureNotFound`, `AbilityNotFound`, `ItemNotFound`, or `MoveNotFound` error when an ID is unregistered.

### Changed

- Changed `PokemonRegistryConfiguration::instantiate()` defaults to generate IVs, natures, abilities, items, and moves when their optional array pointers are null; EVs continue to default to six zero values.
- Factored repeated metadata-array lookup and null detection out of `instantiate()` into `validateMetas()` and four domain-specific validation helpers. This initial revision inverted the `std::expected` success check at the call sites; 0.12.29 corrects the control flow.

## [0.12.25] - 2026-09-11

### Changed

- Changed runtime nature multipliers from one six-stat array shared by all equipped natures to one six-stat multiplier array per nature slot. Pokemon constructors, test fixtures, and call sites now use the nested fixed-array representation.

### Fixed

- Fixed stat recomputation so each `NatureID` maps to and applies its corresponding multiplier row instead of every nature implicitly sharing one row. Pokemon tests were updated to verify the corrected per-nature representation and calculated stats.

## [0.12.24] - 2026-09-11

### Added

- Added the learnset identity and metadata domain: open `LearnsetID`, built-in `None` and one-past-the-end `FinalLearnset`, `AcquisitionMethod` values for level-up, TM, egg, and tutor learning, `MoveLearnEntry`, and owned `LearnsetMeta` name and entry storage.
- Added `PokemonRegistryConfiguration::instantiate()`, which resolves species, nature, ability, item, and move metadata through a non-owning dependency bundle and returns either a constructed runtime `Pokemon` or a domain-specific `RegistryErrorInfo`.
- Added `MoveMeta::mPPMaxAmount` and `mPPDefaultAmount`; built-in move registry metadata and move tests now carry PP information with each move definition.
- Added `MIN_EV_STAT_VALUE` at zero and changed the minimum IV value from one to zero.

### Changed

- Changed `PokemonMeta` from embedding move IDs, maximum PP, a held item, and a logical ability-pool count to referencing a `LearnsetID`; species metadata continues to retain its ability pool, typing, base stats, level, name, and stable Pokemon ID.
- Extended runtime Pokemon construction and fixtures with stable species identity and the IV, EV, nature, ability, item, move, and nature-multiplier inputs required by registry-driven instantiation.
- Updated the example, base-damage tests, Pokemon tests, and move and Pokemon registry tests for zero-minimum IVs, move PP metadata, learnset-backed species metadata, and the initial instantiation API.
...
[0.12.29]: https://github.com/Phaysik/pocketcore/commit/99827bc09682ab31a0e015a411cca39d27f4021b
[0.12.28]: https://github.com/Phaysik/pocketcore/commit/eaec1c1ac71a823dded7ef13f33373cd199fe142
[0.12.27]: https://github.com/Phaysik/pocketcore/commit/dd8e736a5d9440ded6649c06242628f36d20e1eb
[0.12.26]: https://github.com/Phaysik/pocketcore/commit/9924fcfdcf67cf5f762da21f8230054775ca7d64
[0.12.25]: https://github.com/Phaysik/pocketcore/commit/dafe722cb00fab6cd0905bf6a8f61cb9bfd7b564
[0.12.24]: https://github.com/Phaysik/pocketcore/commit/66981e482d09567ebb8dcaf1d2d9a2bf502215c4
